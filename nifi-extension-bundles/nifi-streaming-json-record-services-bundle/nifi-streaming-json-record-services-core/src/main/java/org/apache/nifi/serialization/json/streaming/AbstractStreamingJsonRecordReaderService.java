...
abstract class AbstractStreamingJsonRecordReaderService extends SchemaRegistryService implements RecordReaderFactory {
    ...
    public static final PropertyDescriptor RECORD_MATERIALIZATION_STRATEGY = new PropertyDescriptor.Builder()
            ...
            .description("Specifies when JSON Records are converted into typed values. Deferred postpones conversion of the complete Record until its first field "
                    + "or value access and is optimized for pass-through and grouping. Eager converts fields while reading and is optimized for flows that inspect "
                    + "or modify most Records. Deferred is preferred for eligible direct inferred-schema inputs "
    ...
    public RecordReader createRecordReader(final Map<String, String> variables, final InputStream in, final long inputLength, final ComponentLog logger)
        ...
        } catch (final IOException | MalformedRecordException | SchemaNotFoundException | RuntimeException | Error e) {
            if (replayable != null) {
                try {
                    replayable.close();
