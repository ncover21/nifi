...
final class StreamingJsonParserFactory implements TokenParserFactory {
    ...
    public StreamingJsonParserFactory() {
        ...
        parsingStrategy = ParsingStrategy.STANDARD;
    }
     ...
     */
    public StreamingJsonParserFactory(final StreamReadConstraints streamReadConstraints, final ParsingStrategy parsingStrategy) {
    ...
    @Override
    public JsonParser getJsonParser(final byte[] input, final int offset, final int length) throws IOException {
        Objects.requireNonNull(input, "Input required");
        return jsonFactory.createParser(input, offset, length);
    }

    @Override
    public boolean supportsSerializedJson() {
        return parsingStrategy == ParsingStrategy.STANDARD;
    }

    @Override
    public boolean supportsInputStreamByteOffsets() {
        return parsingStrategy == ParsingStrategy.STANDARD;
    }
}
